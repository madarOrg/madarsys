...
class ModulesSeeder extends Seeder {
    public function run() {
        ...
        // خريطة لتحديد نطاق كل مكون (company/branch/warehouse)
        $scopeMap = [
            'إدارة المستخدمين'         => 'company',
            'إدارة المستودعات'        => 'branch',
            'إدارة المخزون'           => 'warehouse',
            'إدارة الشحنات'           => 'branch',
            'إدارة الفواتير'          => 'company',
            'إدارة طلبات الموردين'     => 'company',
            'إدارة العوائد'           => 'branch',
            'إدارة الموردين'          => 'company',
            'تتبع الكميات'            => 'warehouse',
            'إنشاء التقارير'          => 'company',
            'إدارة العملاء والمحلات'  => 'branch',
            'إدارة مندوبين المحلات'   => 'branch'
        ];

        foreach ($navbarLinks as $section) {
            $key = $this->generateKey($section['text']);
            
            // تحقق من وجود السجل قبل إضافته
            if (!Module::where('key', $key)->exists()) {
                // إنشاء المكون إذا لم يكن موجودًا
                $module = Module::create([
                    'name'        => $section['text'],
                    'key'         => $key,
                    'scope_level' => $scopeMap[$section['text']] ?? 'company',
                ]);

                // إذا كان هناك روابط أو إجراءات، أضفها
                if (isset($section['children'])) {
                    foreach ($section['children'] as $child) {
                        // إنشاء الإجراءات المرتبطة بكل مكون
                        $moduleAction = $module->actions()->create([
                            'name'      => $child['text'],
                            'action_key'=> $this->generateKey($child['text']),
                            'route'     => $child['href'],
                        ]);
                    }
                }
