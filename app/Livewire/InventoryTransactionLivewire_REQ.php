...
class InventoryTransactionLivewire extends Component
{
    ...
    public function save($request)
    {
        try {
            ...
            throw new \Exception("خطأ أثناء حفظ العملية: " . $e->getMessage());

        }
    }

    public function submit(StoreInventoryTransactionRequest $request)
    {DD($request);
        // استدعاء دالة الحفظ
        try {
            $this->save($request);
      ...
      $this->dispatch('alert', type: 'success', message: 'تم حفظ العملية بنجاح!');
        ...
        $this->dispatch('alert', type: 'error', message: 'حدث خطأ أثناء حفظ العملية: ' . $e->getMessage());
