...
class InvoiceController extends Controller
{
    ...
    public function update(Request $request, $type, $id)
    {
        $request->validate([
            ...
            // التحقق من تاريخ الإنتاج
            'items.*.production_date' => ['date', new AfterSystemStartDate()],

            // التحقق من تاريخ الانتهاء
            'items.*.expiration_date' => ['date', new AfterSystemStartDate(), new ValidExpirationDate()],
            ...
            foreach ($request->items as $item) {
                if ($item['item_id'] == 0) {
                    ...
                    if ($invoiceItem) {
                        ...
                        if ($item->unit_id) {
                            $convertedOutQuantity = $this->inventoryCalculationService->calculateConvertedQuantity($item['quantity'], $item->unit_id, $baseUnitId);
                        }
    ...
    public function storeFromOrder(Request $request, $orderId)
    {
        ...
        try {
            ...
            try {
                $inventoryTransaction = $this->storeInventoryTransaction($request, $invoiceCode, $request->items, $transactionType, '');
            } catch (\Exception $e) {
                DB::rollBack();
    ...
    private function isQuantityAvailable($request)
    {
        ...
        if ($requestedTotalAmount <= 0) {
            ...
            throw new \Exception('خطأ في إنشاء الحركة المخزنية: ');
        }
