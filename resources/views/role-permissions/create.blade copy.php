<x-layout>
    <section class=" min-h-screen py-6">
        <form action="{{ route('role-permissions.store') }}" method="POST">
            ...
            <div class="max-w-4xl mx-auto p-6 bg-white dark:bg-gray-800 shadow rounded">
                ...
                <!-- اختيار الدور -->
                <div class="mb-6">
                    <label for="role_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">اختر
                        الدور</label>
                ...
                <div id="permissions-wrapper"></div>
                ...
                <div class="mb-6 grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div id="available-permissions"
                            class="border p-4 bg-white dark:bg-gray-700 rounded-md min-h-[150px]"
                            ondragover="allowDrop(event)" ondrop="dropBack(event)">
                            @foreach ($permissions as $permission)
                                <div class="draggable cursor-move px-3 py-2 bg-gray-200 dark:bg-gray-600 rounded-md mb-2"
                                    draggable="true" data-id="{{ $permission->id }}">
                                    {{ $permission->name }}
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div>
                        <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">الصلاحيات المختارة
                            للدور
                        </h3>
                        <div id="selected-permissions"
                            class="border p-4 bg-gray-100 dark:bg-gray-600 rounded-md min-h-[150px]"
    ...
    <script>
        ...
        function drop(event) {
            ...
            // إضافة الصلاحيات المختارة إلى الحقل المخفي
            updatePermissionsField();
        }

        // دالة لتحديث الحقل المخفي بالصلاحيات المختارة
        function updatePermissionsField() {
            // أولاً، احذف جميع الـ inputs السابقة
            document.querySelectorAll("input[name='permissions[]']").forEach(el => el.remove());

            // أضف input جديد لكل صلاحية مختارة
            const selectedPermissions = document.querySelectorAll('#selected-permissions .draggable');
            selectedPermissions.forEach(item => {
                ...
                document.getElementById('permissions-wrapper').appendChild(input);
        ...
        function dropBack(event) {
            ...
            let permissionId = event.dataTransfer.getData("text/plain");
            let draggedElement = document.querySelector(`.draggable[data-id='${permissionId}']`);
            let targetList = document.getElementById("available-permissions");
    ...
    updatePermissionsField(); // تأكد من تحديث الحقول قبل الإرسال
});
