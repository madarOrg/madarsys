        ...
        <form action="{{ route('purchase_invoices.update', $invoice->id) }}" method="POST">
            ...
            <div class="space-y-12">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="supplier_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">المورد</label>
    ...
    document.getElementById('add-item').addEventListener('click', function () {
        ...
        let newRow = `
            <tr>
                ...
                <td class="py-2 px-4">
                    <input type="number" name="items[${index}][price]" class="price-input w-full py-2 px-4 border rounded" required>
                </td>
                <td class="py-2 px-4">
...
// Event delegation for updating total when price or quantity changes
document.querySelector('#invoice-items-table').addEventListener('input', function (e) {
    if (e.target && (e.target.classList.contains('price-input') || e.target.classList.contains('quantity-input'))) {
        updateTotalPrice();
    }
});
</script>
