          ...
          <table class="w-full text-sm text-right text-gray-500 dark:text-gray-400">
              <thead class="text-xs text-gray-700 uppercase bg-gray-400 dark:bg-gray-700 dark:text-gray-400">
                  <tr>
                      ...
                      <th class="px-6 py-3">آخر تحديث</th>
              ...
              <tbody>
                  @foreach ($products as $product)
                      <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                          ...
                          <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">${{ number_format($product->purchase_price, 2) }}</td>
                          <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">${{ number_format($product->selling_price, 2) }}</td>
                          <td class="px-6 py-4">{{ $product->stock_quantity }} {{ $product->unit }}</td>
                          <td class="px-6 py-4">
                          ...
                          <td class="px-6 py-4">{{ $product->purchaseDate }}</td>
                          <td class="px-6 py-4">{{ $product->manufacturingDate ?? 'غير محدد' }}</td>
                          <td class="px-6 py-4">{{ $product->expirationDate ?? 'غير محدد' }}</td>
                          <td class="px-6 py-4">{{ $product->lastUpdated }}</td>
                          ...
                          <td class="px-6 py-4">{{ $product->WarehouseLocations }}</td>
                          <td class="px-6 py-4">{{ $product->warehouse_id }}</td>
                          <td class="px-6 py-4">
