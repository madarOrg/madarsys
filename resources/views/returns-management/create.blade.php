<x-layout dir="rtl">
    <section class="relative mt-1 flex items-center justify-between">
        ...
        <div class="flex items-center space-x-2 space-x-reverse">
            ...
            <x-button href="{{ route('returns-management.index') }}"
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mt-6">
        <form id="return-form" action="{{ route('returns-management.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- اختيار العميل -->
                <div>
                    <label for="customer_id" class="block text-sm font-medium text-gray-700 mb-2">العميل <span
                            class="text-red-600">*</span></label>
                    <select id="customer_id" name="customer_id"
                        class="tom-select w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                        required>
                ...
                <!-- تاريخ المرتجع -->
                <div>
                    <label for="return_date" class="block text-sm font-medium text-gray-700 mb-2">تاريخ المرتجع <span
                            class="text-red-600">*</span></label>
                ...
                <!-- حالة الإرجاع -->
                <div>
                    <label for="status" class="block text-sm font-medium text-gray-700 mb-2">حالة المرتحع <span
                            class="text-red-600">*</span></label>
                    <select id="status" name="status"
                        class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                        required>
                        <option value="">-- اختر حالة المرتجع --</option>
                        <option value="معلق" {{ old('return_status') == 'معلق' ? 'selected' : '' }}>معلق</option>
                        <option value="مكتمل" {{ old('return_status') == 'مكتمل' ? 'selected' : '' }}>مكتمل</option>
                        <option value="ملغي" {{ old('return_status') == 'ملغي' ? 'selected' : '' }}>ملغي</option>
                    </select>
                    @error('return_status')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    <label for="return_reason" class="block text-sm font-medium text-gray-700 mb-2">سبب الإرجاع <span
                            class="text-red-600">*</span></label>
                    <textarea id="return_reason" name="return_reason" rows="3"
                        class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                        required>{{ old('return_reason') }}</textarea>
                    @error('return_reason')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div id="items-container">
                    <div class="item-row bg-gray-50 p-4 rounded-md mb-4">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">المنتج <span
                                        class="text-red-600">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">الكمية <span
                                        class="text-red-600">*</span></label>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="items[0][return_reason]"
                                    class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                    placeholder="اترك فارغاً لاستخدام سبب الإرجاع العام">
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            addItemButton.addEventListener('click', function() {
                ...
                const itemHtml = `
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                            ...
                            <select name="items[${itemCount}][product_id]" class="tom-select w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                                <option value="">-- اختر المنتج --</option>
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}">
                                       {{ $product->name }}-{{ $product->barcode }}-{{ $product->sku }}
                ...
                if (itemCount > 1) {
                    const firstRemoveButton = itemsContainer.querySelector(
