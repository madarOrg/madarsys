<x-layout dir="rtl">
    <section class="relative mt-1 flex items-center">
        ...
        <!-- مربع البحث -->
        <form method="GET" action="{{ route('returns_process.show', $returnOrder->id) }}">
            <x-search-input id="search-return-orders" name="search" placeholder="ابحث عن المنتجات المرتجعة"
    ...
    <div class="overflow-x-auto bg-white shadow-md rounded-lg mt-4">
        <table class="w-full text-sm text-right text-gray-500">
            ...
            <tbody>
                @foreach($items as $item) <!-- استخدم items بدلاً من returnOrder->items -->
                                @php
                                    // تحديد اللون بناءً على حالة المنتج
                                    $statusColor = '';
                                    switch ($item->status) {
                                        case 'تصنيف كمنتج تالف':
                                ...
                                <tr class="border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600">
                                    ...
                                    <td class="px-4 py-2 w-1/4">
                                        <div class="flex flex-wrap gap-2">
                                            @if($item->status == 'تصنيف كمنتج تالف')
                                                <x-button href="{{ route('return_suppliers.sendToSupplier', $item->product->id) }}"
                                            ...
                                            @elseif($item->status == 'منتهي الصلاحية')
                                                <x-button href="{{ route('return_suppliers.sendToSupplier', $item->product->id) }}"
                                                    class="bg-red-500 text-white p-2 rounded-full hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-400">
                                                    <i class="fas fa-exclamation-triangle text-xs"></i> منتهي الصلاحية
                                                </x-button>
                                            @elseif($item->status == 'قبول الإرجاع')
                                                <x-button
                                                    class="bg-blue-500 text-white p-2 rounded-full hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400">
                                                    <i class="fas fa-sync text-xs"></i> قبول الإرجاع
                                                </x-button>
                                            @elseif($item->status == 'إرجاع للمخزون')
                                                <x-button
                                                    class="bg-green-500 text-white p-2 rounded-full hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-400">
                                                    <i class="fas fa-check text-xs"></i> إرجاع للمخزون
                                                </x-button>
                                            @else
                                                <x-button
    ...
    <!-- استخدام التصفح مع الرابط المناسب -->
