        ...
        <tbody>
            ...
            <tr class="bg-gray-200 border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600">
                ...
                <td class="px-6 py-4 font-medium text-gray-900 dark:text-gray-300">
                    <a href="javascript:void(0)" onclick="togglePartnerDetails({{ $partner->id }})">{{ $partner->name }}</a>
                </td>
                <td class="px-6 py-4">{{ ucfirst($partner->type) }}</td>
                <td class="px-6 py-4">{{ $partner->email ?? 'غير متوفر' }}</td>
                <td class="px-6 py-4">{{ $partner->phone ?? 'غير متوفر' }}</td>
                <td class="px-6 py-4 flex space-x-2">
                    <a href="{{ route('partners.edit', $partner->id) }}" class="text-blue-600 hover:underline dark:text-blue-500">
                    ...
                    <table class="w-full text-sm text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr class="bg-gray-200 border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4">
                                    @if($partner->website)
                                        <a href="{{ $partner->website }}" target="_blank" class="text-blue-500 hover:underline">{{ $partner->website }}</a>
                                    @else
                                        غير متوفر
                                    @endif
                                </td>
