...
<head>
    ...
    <title>طباعة أمر صرف #{{ $salesOrder->order_number }}</title>
...
<body class="bg-gray-100">
    <div class="no-print bg-green-600 text-white p-4 text-center">
        <button onclick="window.print()" class="bg-white text-green-600 px-4 py-2 rounded shadow hover:bg-gray-100">
            <i class="fas fa-print ml-1"></i> طباعة
        </button>
        <a href="{{ route('sales-orders.show', $salesOrder->id) }}" class="bg-gray-800 text-white px-4 py-2 rounded shadow hover:bg-gray-700 mr-2">
    ...
    <div class="print-container bg-white p-8 mx-auto my-8 shadow-lg max-w-4xl">
        ...
        <div class="header flex justify-between items-center mb-8 border-b pb-4">
            ...
            <div class="document-info text-left">
                <h2 class="text-xl font-bold">أمر صرف</h2>
                <p>رقم: {{ $salesOrder->order_number }}</p>
                <p>التاريخ: {{ $salesOrder->issue_date }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-6 mb-6">
            <div class="border p-4 rounded">
                <h3 class="font-bold mb-2 border-b pb-1">معلومات العميل</h3>
                <p>الاسم: {{ $salesOrder->partner->name ?? 'غير محدد' }}</p>
                <p>العنوان: {{ $salesOrder->partner->address ?? 'غير محدد' }}</p>
                <p>الهاتف: {{ $salesOrder->partner->phone ?? 'غير محدد' }}</p>
                <p>البريد الإلكتروني: {{ $salesOrder->partner->email ?? 'غير محدد' }}</p>
            </div>
            <div class="border p-4 rounded">
                <h3 class="font-bold mb-2 border-b pb-1">معلومات الشركة</h3>
                <p>الشركة: شركة مدار</p>
                <p>العنوان: {{ $salesOrder->order->branch->address ?? 'غير محدد' }}</p>
                <p>الهاتف: {{ $salesOrder->order->branch->phone ?? 'غير محدد' }}</p>
        ...
        <!-- معلومات أمر الصرف -->
        <div class="mb-6">
            <h3 class="font-bold mb-2 border-b pb-1">معلومات أمر الصرف</h3>
            <div class="grid grid-cols-3 gap-4">
                <p>رقم الطلب الأصلي: {{ $salesOrder->order_id }}</p>
                <p>تاريخ الإصدار: {{ $salesOrder->issue_date }}</p>
                <p>تاريخ التسليم المتوقع: {{ $salesOrder->expected_delivery_date ?? 'غير محدد' }}</p>
            </div>
        </div>
        ...
        @if($salesOrder->notes)
            <div class="mb-6">
                ...
                <p class="p-2 border rounded">{{ $salesOrder->notes }}</p>
            </div>
        @endif
        ...
        <div class="footer mt-8 text-center text-gray-500 text-sm">
            <p>تم إنشاء هذا المستند بواسطة نظام مدار لإدارة المخزون والمبيعات</p>
