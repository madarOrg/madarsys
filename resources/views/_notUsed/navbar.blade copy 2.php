...
// استخراج الصلاحيات المرتبطة بالدور
        ...
        @foreach ($NavbarLinkss as $menu)
          ...
          @if ($hasPermission)
            <li class="relative group">
              <a href="{{ $menu['href'] }}" class="block py-2 pr-4 pl-3 text-xs text-gray-700 border-b border-gray-100 hover:bg-gray-50 lg:hover:bg-transparent lg:border-0 lg:hover:text-primary-700 lg:p-0 dark:text-gray-400 lg:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white lg:dark:hover:bg-transparent dark:border-gray-700 transition-transform transform hover:scale-110">
                {{ $menu['text'] }}
              </a>
              @if (isset($menu['children']) && count($menu['children']) > 0)
                <ul class="absolute z-50 left-0 hidden w-48 bg-white shadow-lg dark:bg-gray-800 group-hover:block">
                  @foreach ($menu['children'] as $child)
                    @php
                      // التحقق من صلاحيات الوصول للعنصر الفرعي
                      $hasChildPermission = in_array($child['permission'], $permissions);
                    @endphp
