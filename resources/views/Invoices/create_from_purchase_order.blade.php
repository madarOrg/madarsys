<x-layout dir="rtl">
    <section class="relative mt-5 flex flex-col items-start">
        <x-title :title="'إنشاء فاتورة شراء من أمر شراء'"></x-title>
        
        <div class="w-full mt- p-6 rounded-lg shadow-md">
            <div class="mb-6">
                <h2 class="text-xl font-bold mb-2">معلومات أمر الشراء</h2>
                <div class="grid grid-cols-2 gap-4  p-4 rounded-md border border-gray-300">
                    <div>
                        ...
                        <p class="font-semibold">المورد: <span class="font-normal">{{ $purchaseOrder->partner->name ?? 'غير محدد' }}</span></p>
                    </div>
                    <div>
            ...
            <form action="/invoices/store-from-purchase-order/{{ $purchaseOrder->id }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-3 gap-6">
                   ...
                   duration-200 ease-in-out dark:focus:bg-gray-700 focus:outline-blue-500 dark:focus:text-gray-200 mt-1" required>
                            ...
                            @foreach($partners as $partner)
                                <option value="{{ $partner->id }}" {{ $purchaseOrder->partner_id == $partner->id ? 'selected' : '' }}>
                                    {{ $partner->name }}
                                </option>
                            @endforeach
                        </select>
                   ...
                   duration-200 ease-in-out dark:focus:bg-gray-700 focus:outline-blue-500 dark:focus:text-gray-200 mt-1" required>
                            ...
                            @foreach($paymentTypes as $paymentType)
                                <option value="{{ $paymentType->id }}" {{ $purchaseOrder->order->payment_type_id == $paymentType->id ? 'selected' : '' }}>{{ $paymentType->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    {{-- <div>
                        ...
                        <select name="branch_id" id="branch_id" class="tom-select w-full" hidden>
                            ...
                            @foreach($branches as $branch)
                                <option value="{{ $branch->id }}" {{ $purchaseOrder->order->branch_id == $branch->id ? 'selected' : '' }} >
                                    {{ $branch->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="warehouse_id" id="warehouse_id" class="tom-select w-full" required>
                            ...
                            @foreach($warehouses as $warehouse)
                                <option value="{{ $warehouse->id }}"  {{ $purchaseOrder->order->warehouse_id == $warehouse->id ? 'selected' : '' }}>{{ $warehouse->name }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <tbody id="invoice-items">
                            ...
                            <tr  class="item-row bg-gray-200 border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600">
                                <td class="py-2 px-4 ">
                                    ...
                                    <td class="py-2 px-4 ">
                                        <select name="items[{{ $index }}][unit_id]" class="tom-select w-full unit-select" required>
