    ...
    <section class=" p-6  shadow-md rounded-lg">
        <div class="  ">
            <div class="  flex items-center">
                <x-title :title="'فواتير المبيعات'"></x-title>
                <div class="flex "> <!-- Align the button to the left -->
                    <x-button :href="route('invoices.create', ['type' => 'sale'])" type="button">
                        <i class="fas fa-plus mr-2"></i> إضافة فاتورة جديدة
                    </x-button>
                </div>
            </div>
            <!-- Search Form with Filters -->
            <form method="GET" action="{{ route('invoices.index', ['type' => 'sale']) }}">
                        ...
                        <div class="flex flex-wrap justify-between gap-4">
                            ...
                            <div class="flex-1 min-w-[150px]">
                                {{-- <label for="payment_type_id"
                                    class="text-sm font-medium text-gray-600 dark:text-gray-400">طريقة
                                    الدفع</label>
                                <select name="payment_type_id" id="payment_type_id"
                                    class="w-full bg-gray-100 rounded border py-1 px-3 leading-8">
                                    <option value="">اختر طريقة الدفع</option>
                                    @foreach ($paymentTypes as $paymentType)
                                        <option value="{{ $paymentType->id }}"
                                            {{ request()->input('payment_type_id') == $paymentType->id ? 'selected' : '' }}>
                                            {{ $paymentType->name }}
                                        </option>
                                    @endforeach
                                </select> --}}
                        ...
                        <tr
                            ...
                            <td class="px-6 py-4 flex">
                                <div class="flex items-center space-x-3 rtl:space-x-reverse">
                                    ...
                                    <a href="{{ route('invoices.edit', ['type' => 'sale', 'invoice' => $invoice->id]) }}"
                                        class="text-blue-600 hover:underline dark:text-blue-500">
                                        <i class="fa-solid fa-pen"></i>
                                    </a>

                                    <form
                                        action="{{ route('invoices.destroy', ['type' => 'sale', 'invoice' => $invoice->id]) }}"
