        ...
        <div class="mb-6">
            <form action="{{ route('shipments.track.index') }}" method="GET" class="flex space-x-4 space-x-reverse">
                <div class="w-1/3">
                    ...
                    <input type="text" name="search" id="search" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" value="{{ request('search') }}">
                </div>
                <div class="w-1/3">
                ...
                <table class="min-w-full bg-white border border-gray-300">
                    ...
                    <tbody>
                        @foreach($shipments as $shipment)
                            <tr class="{{ $shipment->status == 'received' ? 'bg-green-50' : ($shipment->status == 'shipped' ? 'bg-blue-50' : '') }}">
                                ...
                                <td class="py-2 px-4 border-b text-center">{{ $shipment->received_date ?? 'لم يتم الاستلام بعد' }}</td>
