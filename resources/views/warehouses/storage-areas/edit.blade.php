<x-layout>
    <section class="bg-gray-50 dark:bg-gray-900">
        <form action="{{ route('warehouse.storage-areas.update', ['warehouse' => $warehouse, 'storage_area' => $storageArea]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="space-y-12 dark:bg-gray-900 mb-24">
                <div class="pb-12">
                    <x-title :title="'تحديث منطقة التخزين'" />
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">يرجى تعديل بيانات منطقة التخزين بدقة لضمان تنظيم البيانات.</p>
                    ...
                    <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 min-h-full">
                        ...
                        <div class="col-span-1">
                            <x-file-input id="area-name" name="area_name" label="اسم المنطقة التخزينية" type="text" 
                                placeholder="أدخل اسم المنطقة" required value="{{ old('area_name', $storageArea->area_name) }}" />
                            @error('area_name')
                        ...
                        <div class="col-span-1">
                            <x-file-input id="area-type" name="area_type" label="نوع المنطقة التخزينية" type="text" 
                                placeholder="اختر نوع المنطقة" required value="{{ old('area_type', $storageArea->area_type) }}" />
                            @error('area_type')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="col-span-1">
                            <x-file-input id="capacity" name="capacity" label="السعة القصوى للتخزين" type="number" 
                                placeholder="حدد السعة" required value="{{ old('capacity', $storageArea->capacity) }}" />
                            @error('capacity')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="col-span-1">
                            <x-file-input id="current-occupancy" name="current_occupancy" label="عدد المنتجات المخزنة" type="number" 
                                placeholder="حدد العدد" required value="{{ old('current_occupancy', $storageArea->current_occupancy) }}" />
                        ...
                        <div class="col-span-1">
                            <x-select-dropdown 
                                ...
                                :options="$zones->toArray()"  
                                :selected="old('zone_id', $storageArea->zone_id)" 
                        ...
                        <div class="col-span-1">
                            <x-file-input id="storage-conditions" name="storage_conditions" label="شروط التخزين" type="text" 
                                placeholder="حدد شروط التخزين" required value="{{ old('storage_conditions', $storageArea->storage_conditions) }}" />
