            ...
            <div x-data="{ open: false }"> <!-- تغيير القيمة من true إلى false -->
                ...
                <div x-show="open" x-transition>
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                        ...
                        <x-select-dropdown id="distribution_type" name="distribution_type" label="النوع"
                            :options="[
                                '' => 'الكل',
                                '1' => ' إدخال مخزني',
                                '-1' => 'إخراج مخزني',
                            ]" :selected="request('distribution_type')" onchange="this.form.submit()" />
                        ...
                        <x-select-dropdown id="storage_area_id" name="storage_area_id" label="المنطقة التخزينية"
                            :options="$storageAreas->pluck('area_name', 'id')" :selected="request()->storage_area_id" required />
                        ...
                        <!-- تواريخ الإنتاج -->
                        <x-file-input id="production_date_from" name="production_date_from" label="إنتاج من:"
                            type="date" value="{{ request('production_date_from') }}" />

                        <x-file-input id="production_date_to" name="production_date_to" label="إنتاج إلى:"
                            type="date" value="{{ request('production_date_to') }}" />
                        ...
                        <x-file-input id="expiration_date_to" name="expiration_date_to" label="انتهاء إلى:"
                            type="date" value="{{ request('expiration_date_to') }}" />
                ...
                @if ($products->isNotEmpty())
                    <div class="overflow-x-auto mt-0">
                        ...
                        <table class="w-full text-sm text-right text-gray-500 dark:text-gray-400">
                            <thead
                                class="text-xs text-gray-700 uppercase bg-gray-400 dark:bg-gray-700 dark:text-gray-400 p-2 w-auto min-w-[50px] whitespace-nowrap">
                                <tr class="">
                            ...
                            <tbody>
                                @foreach ($products as $product)
                                    <tr
                                        ...
                                        <td class="p-2 w-auto min-w-[50px] whitespace-nowrap">
                                            {{ $product->warehouse_name ?? 'غير محدد' }}</td>
                                        ...
                                        <td class="p-2 w-auto min-w-[50px] whitespace-nowrap">
                                            {{ $product->productQuantity ?? 'غير محدد' }}</td>
                                        <td class="p-2 w-auto min-w-[50px] whitespace-nowrap">
                                            {{ $product->location->rack_code ?? 'غير محدد' }}</td>
                                        <td class="p-2 w-auto min-w-[50px] whitespace-nowrap">
                                            {{ $product->storageArea->area_name ?? 'غير محدد' }}</td>
                                        <td class="p-2 w-auto min-w-[50px] whitespace-nowrap">
                                            {{ $product->production_date ?? 'غير محدد' }}</td>
                                        <td class="p-2 w-auto min-w-[50px] whitespace-nowrap">
                                            {{ $product->expiration_date ?? 'غير محدد' }}</td>
                                        <td class="p-2 w-auto min-w-[50px] whitespace-nowrap">
                                            {{ $product->batch_number ?? 'غير محدد' }}</td>
                                        <td class="p-2 w-auto min-w-[50px] whitespace-nowrap">
                                            {{ $product->inventory_transaction_item_id ?? 'غير محدد' }}
                                        </td>
                                        <td class="p-2 w-auto min-w-[50px] whitespace-nowrap">
